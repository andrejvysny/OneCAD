...
- `reserve(bytes, reason, expectedBytes?)` returns a RECEIPT or null; `bytes` is the PEAK held against the ceiling and `expectedBytes` the resting size `put` admits EXACTLY (`!==` refuses — the peak is ~2× after growth and would hide a plan/build drift). Price with `planFaceSetCapacity(entry, ordinals, reuse)`: reserve `plan.peakBytes`, expect `plan.bytes`. The old `estimateFaceSetBytes` (triangles × 3 × 16 B) was a lower bound the ×1.5 growth rule beats on every growth step (4→5 triangles: 240 vs 288) and is deleted. `OwnedFaceGeometry.update` calls the same planner, so plan and allocation cannot drift, and it THROWS if the wrapper was disposed.
- `HighlightCache.put` refusals are non-destructive: both checks run before the same-key slot is touched, and a PINNED slot is never replaced (refuse, don't drop) — something is drawing it.
- Degraded highlight = leased `entry.edgeGeometry` outline + a count chip via `HighlightDeps.onDegraded` → `ViewportEngine.syncDegradedChips`; re-attempted on every rebuild, and `HighlightLayer.degraded` reads the DISPLAY state. `HighlightDeps.budget` exists so a test can exhaust it. A body with NO edges falls back to the whole-body tint but keeps the same `DegradedSelectionNotice` — build the notice once and spread it onto whichever overlay is returned, or the fallback silently stops counting, stops reading `degraded`, and sticks on screen via the survivor rule.
- Notice equality must include `world` (the chip anchor) or a moved body keeps its chip at the old position.
