...
## SKETCH-V2 — Constraint System V2 (plan approved 2026-08-15, `~/.claude/plans/act-as-senior-frontend-warm-valley.md`)
Normative spec: user's "OneCAD Sketch Constraint System — Hardening Specification" (§1–§60). Design goal: *geometry is manipulated directly; the pointer expresses intent; the solver decides what movement is possible from the remaining DOF.*
Root defect: the frontend has NO point topology. `SketchEntity` (`src/ipc/types.ts:126-150`) inlines coordinates, so a point is a raw pair or an `{entityId, position}` role label re-resolved on every use, and "same point?" is answered by float proximity in **10 sites / 6 tolerances**. Connected endpoints are separate points + `Coincident`, not shared topology — while `onecad-core/src/sketch/entity.rs:1-18` already carries the real model ("shared point identity is what makes coincident endpoints and rubber-band drags work").
Phases: P0 pin contract → P0.5 identity primitives → P1 auto-constraint restriction → P2 atomic edit txn + diagnostics (SCHEMA rev 1) → P3 arc endpoint addressing (SCHEMA rev 1) → P4 canonical shared topology (SCHEMA rev 2) → P5 connected selection → P6 solver mobility (nullspace) → P7 solver-native drag → P8 drag handles → P9 anchoring → P10 per-entity state → P11 selection halo → P12-16 candidates/dimensions/inspector/ellipse.
**Verified corrections to the spec (change its cost):**
- **C1 (large de-risk):** arc endpoints are ALREADY real `SketchPoint` entities in the worker (`Sketch.cpp:1770-1773`) and `WireSketch.cpp` resolves point slots generically for EVERY kind (`:376-462`). The `Coincident`-only restriction is entirely Rust (`constraint.rs:94-107`, `wire.rs:3401`). Spec Phase 5 = Rust + SCHEMA prose, **worker gains coverage not code**; fields are `serde(default, skip_serializing_if)` so extension is byte-identical (**no fixture bump** — verify via `sketch_freeze` + `wire_contract`). Moved early to P3.
- **C2:** `solveWithGroupDrag`/`beginGroupDrag`/`restoreGroupDragStartPose` already exist (`Sketch.cpp:1369/1392/1403`) — strict rigid group drag (§10/§11) is wiring, not a solver build.
- **C3:** pin-all confirmed (`Sketch.cpp` `beginPointDrag`); pins AND `Coincident` are both driving (`ConstraintSolver.cpp:798-801,1125`), so welded-junction drag is over-determined and **broken today**; the rectangle escape needs shared point ids the FE never produces, so it never fires (`TODO.md:6405`).
- **C4 (ordering hazard):** ellipses are not registered with PlaneGCS (`Sketch.cpp:1805`) → naive DOF fallback, explicitly wrong under redundancy. Spec puts ellipse parity LAST but P10 per-entity color needs trustworthy DOF ⇒ surface `analysisIncomplete` in UI from P10, and DoD excludes ellipse sketches until P16.
- **C5 (GAP — spec does not address):** the mock lane cannot compute analysis/mobility (`localSolver.ts:23-33,315` one-key echo, no propagation; `mockSketch.solveSketch:128-135` returns no positions) yet drives ALL vitest + e2e. Resolution: `analysisIncomplete: true` on the mock lane, interaction truth tested worker-backed (`src-tauri/tests/` + ctest), Playwright asserts plumbing only. **Never build a mini-solver into the mock** (would be a second silently-divergent constraint engine — §15's own failure mode).
**Supersedes earlier session decisions** (deliberate, recorded): derived index → real shared topology; frontend-only → all four layers; vertex render/selection-only → solver-native drag; no-migration → legacy documents stay valid (§7); union-find identity → shared vertices primary with `Coincident` kept for legacy + reference-ownership boundaries (§5/§56).
Adjacent bugs folded in: drag/Delete resurrection (`finishDrag:2775-2781` setSessions a pre-delete array; drag begin/end are NOT on `enqueueSketchMutation` while commit `:1811`/mirror `:2169` are) → P7; free `Point` mints two unmergeable identities (`sketchWireMap.addEntityOps:271-272` keys both `.Start` and `.Center`) → P0.5; `autoConstrain` partner choice is array-order dependent (`:457-459`) → P1.
- [x] P0a closedLoop characterization pin (2026-08-15): `closedLoop.test.ts` NEW (module had zero tests; sole consumer `SketchObject.ts:628-629`). 20 cases pin the behaviour P4 must preserve — degree>2 branch exclusion incl. rectangle-with-tail losing its loop, `chain.length >= 3` closure minimum, `entities.length + 1` walk guard, failed-chain release, Line/Arc-only eligibility, EPS 1e-6 weld (1e-9 welds / 1e-3 does not), `loopCentroid` null paths. **Two LIMITATIONS pinned as such, expected to change under shared topology:** a 2-entity closed profile (line + arc D-shape) is NOT detected, and `loopCentroid` assumes entities are stored in walk order. 20/20 first run.
- [x] P0b constraint-v2 golden corpus (2026-08-15): 13 declarative cases (free/H/V/fully-constrained line, connected L ±anchor, branched polyline, rectangle, origin anchor, circle, arc, redundant, conflicting) carrying `targetDof` + `currentMockDof` + vertices/connectedComponent/entityStates/drags/edits. **Placed at `src/test/constraint-v2/`, not the spec's `tests/constraint-v2/`** — vitest `include` is `src/**/*.{test,spec}.{ts,tsx}` (vite.config.ts:114) so a root dir needs a config change, and `corpus/` is reserved for OneCAD-CPP oracle captures carrying provenance citations (this is forward spec, not capture). Sibling of `src/test/contracts/`. Runner is PHASE-GATED: `LANDED_PHASES`={P0} asserts well-formedness + mock DOF; P2/P4/P7/P10 blocks skip. Three self-defences so a gate cannot be passed by editing one constant: `WIRED_PHASES` guard (claiming a phase with no assertion block fails), stray-field guard (a fixture field with no route here fails), and `notWiredYet()` placeholder bodies that THROW rather than `toBeDefined()`. `KNOWN_DOF_GAPS` is empty — the naive mock count agrees with the spec target on all 13 cases, which is why DOF is the one lane P0 can already assert; an unlisted divergence now fails.
- [x] P0 gate (2026-08-15): tsc 0 · FE **4620 passed / 78 skipped / 280 files** (78 = the phase-gated corpus blocks) · ctest **131/131** · cargo **0 failed** across all targets (worker-backed, `ONECAD_REQUIRE_WORKER=1`) · hex gate **clean** (the previously-tracked inputProbe.ts hit is gone) · e2e **421 passed / 5 failed**. Zero production change — P0 is test-only.
- **e2e failures are NOT from P0 — pre-existing on HEAD `5db4eef`** ("fix(sketch): UX hardening pass — fused toolbar…", landed mid-session from a concurrent session; it committed the 21 files that were dirty at session start). Proof P0 cannot be the cause: the only added production file (`sketchTopology.ts`) is imported by nothing, and test files are not bundled — adding unreachable files cannot change app behavior. No `git stash` used to prove this (concurrent session live; stashing would sweep its work).
- **FLAGGED REGRESSION for the toolbar/fillet owner (outside SKETCH-V2 scope, not fixed here):** `e2e/filletChamfer.spec.ts:181` + `:211` fail on chromium AND webkit — reproduced 2/2 on a targeted re-run, so not flake. `pageerror.log` is EMPTY (no exception) — it is a behavioral break: `expect(received).toBe("Chamfer")` / `Received: "Fillet"`, i.e. the bisector-tier arm is no longer direction-driven (drag INTO the body must type Chamfer) and an explicit segment pick no longer LOCKS the type against a later drag. Fifth failure `e2e/sketch-multi-object.spec.ts:44` is webkit-only. Not touched from this WP to avoid colliding with the concurrent session's in-flight work.
### P0.5 — identity primitives (in progress)
- [x] `src/tools/sketch/sketchTopology.ts` NEW + 38 tests: `pointKeyOf`/`parsePointKey` (ONE minter, ONE parser — replaces the 4 independent spellings of `${entityId}.${position}`), `canonicalRole` (the `Point.Center → Point.Start` alias), `entityPointSlots`/`pointWireForm` (the canonical slot table the 4 existing tables re-declare), strict `pointCoordOf`, `coordOfPointKey`. Addressing only — the vertex graph is P4.
- [x] **R9 evidence — the exhaustive (type × position) migration table.** Proven cell by cell: the strict resolver NARROWS exactly 10 cells the lenient `sketchWireMap.pointCoord` used to answer (`Point.End`, `Point.Midpoint`, `Line.Center`, `Circle.Start/End/Midpoint`, `Ellipse.Start/End/Midpoint`, `Arc.Midpoint`), WIDENS exactly 1 (`Point.Center`, the deliberate alias), and NEVER changes a non-null answer (narrowing only — no silent re-binding). The two strict twins (`badgeLayout.entityPointCoord`, `dimensionLineLayout.resolvePoint`) are proven to agree with each other on all 20 cells; both are reproduced literally in the test so it survives their deletion and cannot go tautological.
- Hydration audit (R9): `frontendConstraintsFromDto.wirePosition` (`sketchWireMap.ts:1269-1271`) accepts ANY of the 4 positions on ANY entity type, so an out-of-table combo IS reachable from the wire — the narrowing table above is the guard, not a formality.
- Seam found: `frontendSolvedPositions:1021` reverse-maps first-wins, so a free `Point`'s two addresses resolve by `Map` insertion order today (`.Start` wins because `addEntityOps:271-272` seeds it first). The alias makes that explicit rather than incidental.
- [x] Dedup landed: `badgeLayout.entityPointCoord` + `dimensionLineLayout.resolvePoint` + `sketchWireMap.pointCoord` all delegate to `pointCoordOf`; `sketchWireMap.pointKey` + `selectGesture.pointRef` mint through `pointKeyOf`. **`defaultPointRole` was required** — the retired `pointCoord` took `position?` and FELL BACK per type, and `Fixed` is its only call site (`toWireConstraint:506`) where a null coord DROPS the constraint; without preserving the absent-position path, hydrated `Fixed` constraints would have vanished. Proven by a test asserting strict+default ≡ lenient for every type with no position. Stale "kept local so this ipc-layer module stays free of a tools/sketch import" comments replaced (the edge is deliberate; precedent: `mockSketch.ts:19`, `localSolver.ts:60`, `mockClient.ts:67-83`).
- [x] **Test-design catch:** the widening proof originally IMPORTED `entityPointCoord` from production, so rewiring production made it compare the new implementation with itself — it failed loudly and is now a literal copy. Both legacy resolvers are reproduced in the test so the migration evidence survives their deletion. This is the trap the test header warns about, caught by its own guard.
- [x] Loud drop: `marshalUpsert` gained `onDrop?` (additive) + `DroppedConstraint{id,type,reason}`. **The user-facing guard is `constraintMarshalBlocker`, NOT the callback** — `SketchIdMap` lives in the tauri client's closure, so an id-map-based check is invisible to vitest and every Playwright spec. The blocker is pure `(entityType, position)`, so both lanes agree, and `applyConstraintNow` now refuses BEFORE any upsert with an explicit hint instead of upserting a constraint the marshaller silently discards. Reasons: `arcEndpointRole` (P3 lifts it), `derivedPoint`.
- [x] P0.5 gate (2026-08-15): tsc 0 · FE **4676 passed / 281 files** · 48 topology tests.
### P1 — auto-constraint restriction (spec §35-§39)
- [x] **Policy gate, not deletion.** `AUTO_KINDS_V2` = {Horizontal, Vertical, Perpendicular, Coincident, Fixed}; `AUTO_KINDS_LEGACY` adds Parallel/Tangent/Concentric/Equal. `InferOptions.kinds` defaults to V2 so a new caller gets safe behaviour without opting in. The legacy rules stay fully implemented because `autoConstrain.test.ts` is the C++ `AutoConstrainer` parity record — deleting them destroys that evidence. The 8 parity tests now request `AUTO_KINDS_LEGACY` explicitly, which also documents "legacy, not current policy". `SketchController.buildCommitConstraints:1882` passes no `kinds` ⇒ V2 is live in production.
- [x] **Coincident deliberately RETAINED** (§36): it is today's weld between chained segments. Removing it before shared vertices would leave polylines visually touching and mechanically falling apart. It goes in P4, not before.
- [x] Determinism fix: the coincidence partner was `refs.find(...)` — first ARRAY-ORDER match, so a new point landing on an already-welded junction bound to a partner decided by the order of `existing`. Now nearest-wins with the point address as tie-break; pinned by an order-invariance test.
- [x] Alt suppression (§38) — **half of it already existed**: `computeSnap` already receives `suppress: this.altHeld` and dimension rounding was already gated (`SketchController:1235,1253,1282`, tracked by keydown/keyup at `:2988/:3089`). My first attempt duplicated the field and re-gated the snap flags from pointer events, which would have fought the established keyboard tracking — reverted. The genuinely missing half was the CONSTRAINT side: an Alt-held placement could still come back welded or Horizontal. Now `kinds: EMPTY_KINDS` at commit. **Required gating H/V, the origin `Fixed`, and `Coincident` on the allowlist too** — they were ungated, so the empty set would have suppressed nothing.
- [x] §39 redundancy filtering: already satisfied by the existing precedence (H/V blocks Perpendicular/Parallel via `hasOrientationConstraint` parity; `seenPairs` dedups; one partner per point). No new machinery invented. The remaining case — skip a Coincident between points already welded — needs the vertex graph and belongs to P4.
- [ ] §37 visible inference preview (drawing shows the candidate before mouse-up) — UI work, not started
### P3 — arc endpoint addressing (spec Phase 5, pulled early per C1) · SCHEMA revision 1
**The headline defect closed: a constraint on an arc endpoint no longer silently vanishes.**
- [x] **C1 confirmed in practice — the C++ worker needed ZERO changes.** `WireSketch.cpp` already routes every kind's point slots through one `resolve_point`; the restriction was entirely Rust (`constraint.rs` carried `point*_position` on `Coincident` alone, `wire.rs:3401` emitted `positions` only for it).
- [x] Rust core: `CurvePosition += Center`; position fields added to the 7 point-taking variants (`Fixed`, `Midpoint`, `OnCurve`, `HorizontalDistance`, `VerticalDistance`, `Symmetric`, `Distance`), all `#[serde(default, skip_serializing_if = "CurvePosition::is_arbitrary")]`. **`constraint_with_value` now carries the positions THROUGH a value edit** — dropping them would have silently re-bound an arc-endpoint dimension to the arc's centre the first time the user retyped its value.
- [x] `wire.rs`: one `point_roles()` helper emits `positions` only when some slot is non-Arbitrary, so pre-P3 constraints stay byte-identical. `OnCurve` emits `positions[0]` = the POINT's role, `positions[1]` = the CURVE position — deliberately distinct slots.
- [x] **No fixture bump — PROVEN, not assumed:** `sketch_freeze` 4/4 green with zero golden edits (`every_constraint_kind_is_snapshot_locked`, `full_sketch_round_trips_byte_stable`). 129 construction sites across 40 files were mechanically patched with `CurvePosition::Arbitrary`.
- [x] Frontend: the arc-aware `slot()` resolver hoisted out of `Coincident` and applied to every point-taking kind; `WireConstraint` shapes gained optional `*Position` fields; `constraintMarshalBlocker` lost `arcEndpointRole` (only `derivedPoint` remains); `marshalsAsPoint` now accepts an Arc's Start/End.
- [x] **Two defect-pinning tests FLIPPED** (each documented in place): `sketchWireMap.test.ts` "an arc Start/End under a kind OTHER than Coincident is still skipped" → "is now marshalled"; `constraintApplicability.test.ts:232` "EXCLUDED: an Arc's Start/End (unmappable)" → "INCLUDED". Added a companion asserting a NON-arc slot still emits no role, so the wire shape is provably unchanged for ordinary points.
- [x] SCHEMA §7.3: normative **`PointRef`** definition (point-entity id OR owner+role over the registered handles), `Center` added to the role vocabulary, the **empty-role convention made normative** (absent ≡ `""` ≡ "this slot IS a point" — implemented on both sides for months, documented nowhere), `OnCurve`'s two position meanings disambiguated, reader-must-fail-loudly generalized. §14 changelog entry written in house style.
- [x] Worker-backed proof: `sketch_constraints.rs::distance_to_an_arc_endpoint_solves_through_real_worker` — a `Distance` from a free point to `<arc>.start` is ACCEPTED and reduces DOF through the real worker. Coverage the seam never had (the C++ side could always do this).
- [x] P1+P3 gate (2026-08-15): tsc 0 · FE **4679 passed / 281 files** · cargo **1260 passed / 0 failed** (worker-backed) · clippy `-D warnings` **0** · fmt clean · ctest **131/131** · hex gate clean · e2e **422 passed / 4 failed**, the 4 being the pre-existing `filletChamfer` breakage from `5db4eef` (one MORE test passes than the pre-work baseline of 421 — the 5th earlier failure, webkit `sketch-multi-object`, was a flake).
- Flagged: an earlier e2e run was launched mid-edit and then raced by a `pkill`, producing `page.goto` failures — discarded as unusable rather than reported as a result.

### P4 — canonical shared topology · SCOPED, NOT STARTED
Two findings that reshape this phase. Both verified, both should be read before anyone begins.
- **The Rust document model ALREADY IS the target model.** `onecad-core/src/sketch/entity.rs:96-108` — `Line { start: EntityId, end: EntityId }`, and Circle/Arc/Ellipse reference a `center` EntityId. This is exactly spec §3's `SketchLine { start: VertexId, end: VertexId }`. The worker wire already carries `p0Ref`/`p1Ref`/`centerRef` (`wire.rs:2722-2730`), and `ownershipFromWire` (`sketchWireMap.ts:1004`) already parses them. **No SCHEMA revision 2 is needed for the document format, and no Rust core change** — the same shape of de-risk as C1.
- **But the frontend never AUTHORS shared points, so the sharing does not exist yet.** `sketchWireMap.ts:24-26` states it plainly: *"the frontend Coincident/adjacency model gives each line its OWN endpoints tied by Coincident."* Two chained lines marshal to FOUR points + a Coincident, never two points. So a vertex graph cannot be read off the wire refs today — the refs are all distinct. Real shared topology requires changing what the frontend **authors**, which is the actual P4 work.
