 ...
 * each picked element's factual readout, the BODY's mass properties (volume /
 * surface area / centroid), and the derived plane relationship (angle, or
 * offset when the two faces are parallel). Pair values belong to both picks,
 * so pinning them to one pick's centre would misattribute them.
 *
...
import { deltaReadout, pickReadout, planePairLabel } from "./measurePresentation";
import { AnnotationControls } from "./AnnotationControls";

export { planePairLabel } from "./measurePresentation";
...
function Row({ label, value, testid }: { label: string; value: string; testid: string }) {
  return (
    <div className="flex min-w-0 items-baseline justify-between gap-4">
      ...
      <span
        className="min-w-0 break-words text-right font-mono text-ink-2"
        data-testid={testid}
...
export function MeasurePanel() {
  ...
  const fence: GeometryReadFence | null =
    ...
    publishedRuntimeSession === runtimeSession &&
  ...
  const fenceKey = fence
    ? `${fence.documentId}:${fence.runtimeSession}:${fence.snapshotId}`
    : null;

  useEffect(
    () => createClient().onDocumentChanged(() => publicationChanged()),
    [],
  );

  useEffect(() => {
    if (!bodyId || !fence) return;
    // Already loaded for this body — a re-pick on the same solid must not refetch.
    const cached = measureStore.getState();
    if (
      cached.mass?.bodyId === bodyId &&
      cached.massFence?.documentId === fence.documentId &&
      cached.massFence.runtimeSession === fence.runtimeSession &&
      cached.massFence.snapshotId === fence.snapshotId
    ) return;
    let live = true;
    void createClient()
  ...
  const showMass =
    ...
    mass.bodyId === bodyId &&
    fence !== null &&
    massFence?.documentId === fence.documentId &&
    massFence.runtimeSession === fence.runtimeSession &&
    massFence.snapshotId === fence.snapshotId;

  return (
    <div
      data-testid="measure-panel"
      className="w-full min-w-0 max-w-full rounded-md border border-border bg-surface px-3 py-2.5 text-[11.5px] shadow-panel"
    >
      ...
      <div className="flex flex-col gap-1">
        ...
        {summary && (
          <>
            ...
            />
            <Row label="Delta" value={deltaReadout(summary)} testid="measure-panel-delta" />
          </>
        )}
        {pairLine && (
