...
- Math-, geometry-, numerical-policy-, or algorithmic-cost-dominant questions (tolerance derivation, blend continuity, sweep/loft framing, identity scoring, solver conditioning, robust predicates, prior art, complexity of a kernel path) → `/codex-astra` is the FIRST step, not a last resort: `derive` before the design, `break` after the derivation or the kernel diff, plus `verify`, `research`, `perf`, and `followup`. Propose the run yourself with the skill's approval summary and wait for the go — the account is ChatGPT Pro 5×, so the budget is roughly three calls per work package with a fan-out of at most two on distinct questions, and a kernel question is repository-grounded by default on a named read-only allowlist. Skill: `.claude/skills/codex-astra/SKILL.md`; accepted derivations live in `docs/design/astra/` and are the provenance citation for any number derived from them.
...
- **GPT-6 Astra (ChatGPT Pro 5×, via `/codex-astra`) — research scientist and algorithm architect, then adversarial reviewer.** Owns the mathematics before code and the attack after it: tolerance derivation and epsilon scaling laws, robust predicates, fillet feasibility and G0/G1/G2 blend budgets, curve/curve and region intersection, sweep framing (Frenet, rotation-minimising, fixed-normal), loft correspondence and seam placement, helical and thread geometry, PlaneGCS conditioning, topology-identity scoring theory, numerical optimisation, property and invariant test design, prior-art and documented-OCCT-behaviour research, algorithmic performance design, and adversarial review of kernel, identity, and numerical work. Astra never writes code; on a kernel question it may read a named read-only allowlist of at most twelve paths, and otherwise reasons over the packet alone. Fable writes the packet, verifies every number, and implements.
- Interactive and GUI QA stays **Fable** with Playwright and wdio. `codex exec` is headless and exposes no computer-use tool, so the model-comparison report's QA recommendation does not route to Astra from here.
...
Routing rule for a work package: if the deliverable is a formula, invariant, predicate, frame, correspondence rule, scoring function, cost model, or documented external fact, Astra derives first and Fable implements; if it is code across layers, Fable implements and Astra breaks when the package is kernel-, identity-, or numerically critical. Everything else is Fable alone. Expect roughly a third of the high-value reasoning to go to Astra, and inside `worker/src/kernel` expect most algorithm design to — that is an expectation about where the work naturally lands, **not** a quota to hit; routing is decided by the shape of the deliverable, never by a percentage.

**Astra-first domains.** These are the parts where OneCAD is trying to become more than a wrapper around OCCT, and they get an Astra derivation *before* implementation rather than an Astra post-mortem after a failure: fillet robustness, blend algorithms, tolerance theory, topology identity, curve/curve and region intersection, solver mathematics, Sweep, Loft, modeled threads, direct modeling, and any other novel geometry algorithm. Being aggressive here is the point — a derivation that costs one call is cheaper than a wrong epsilon that reaches `corpus/expected-values/`.

**The loop.** For everything in that list, run all three steps in order:

1. **Astra discovers and derives** — `/codex-astra derive` produces the invariants, predicates, epsilon policy with its scaling law, degeneracy classes, algorithm sketch, and test vectors. Fable writes the packet and verifies every number that comes back. The accepted result lands in `docs/design/astra/<topic>.md` and becomes the DESIGN section of the implementation brief.
2. **Fable builds** — Fable implements across every layer the change touches (C++ op, `protocol/SCHEMA.md` and its fixtures, Rust integration, frontend, tests) and runs the gate ladder. Astra never writes code and never closes a gate.
3. **Astra tries to break it** — `/codex-astra break` attacks the derivation and the resulting diff on a fresh packet: counterexamples, proof gaps, missing degeneracy classes. Its findings become red-first fixes. This is an *additional* fresh-context review, not a replacement for the local `adversarial-reviewer`.

**Worked example, the WP-G shape.** A fillet-acceptance-policy question — "at what residual is a section genuinely wrong rather than merely approximated" — is led by Astra at `max`, because the deliverable is an epsilon policy with a scaling law and a degeneracy taxonomy, not code. Implementing that policy across the C++ op, the protocol fixtures, the Rust integration, and the complete gate ladder goes back to Fable at `xhigh`. Fable stays the main coding agent throughout: the repository's difficulty is long-horizon repository-scale state, which is where Fable's evidence in this project already sits, and no part of the loop hands implementation to Astra.
...
| New geometry algorithm or tolerance policy | `xhigh` (implement) | `derive` at `max` |
| Unknown OCCT behaviour or prior art | `high` (integrate) | `research` at `high`, live web |
| Hot kernel path, before profiling | `high` (measure) | `perf` at `high` / `xhigh`, grounded |
| A derivation left an unknown open | `high` | `followup` at the parent run's effort |
| Hardest root cause with no repro | `max` | `verify` at `high` on the suspected derivation |
| Hardest derivation in a program | `xhigh` | `ultra` on explicit approval only — multi-agent, burns more than one call |

Astra runs on ChatGPT Pro 5× windows, so judgement replaces rationing: about three calls per work package (`derive`, `break`, and one `followup` or a second `break` on a different dimension), a fan-out of at most two and only on distinct questions, never the same packet to two models. The CLI exposes 272 K of context and no per-token cost, so packet length is not the constraint — packets are capped at 120 KB for readability. A kernel question is repository-grounded by default (read-only, at most twelve named paths, and never `corpus/expected-values/` on a `derive`, which would let the derivation read its own answer); everything else stays prompt-only. When Astra names an unknown that changes the answer, the move is a `followup` round, not a guess by either model.
...
- **Zustand store pattern** (copy `src/stores/appStore.ts`): vanilla `createStore<State>()((set, get) => ({…}))` plus a typed `useXxxStore<T>(selector)` hook in the same file; export the mechanical pair `xxxStore` / `useXxxStore`. Only `settingsStore` is persisted (`persist`, key `onecad.settings`, version 11 — bump and migrate on reserved-field removal). No `devtools` middleware. Cross-store access via direct imports plus `store.getState()` or `.subscribe()`.
...
- Clippy-clean with `-D warnings` — on the NEWEST stable, not just the local toolchain: CI installs `stable` unpinned, so a new release's lints go red there first (2026-09-02: 1.98's `chunks_exact_to_as_chunks`); before pushing, `rustup toolchain install <stable> -c clippy` and `cargo +<stable> clippy --workspace --all-targets -- -D warnings` on a scratch `CARGO_TARGET_DIR`. Prefer `&str` over `String` for params; `#[tracing::instrument]` on arg-identifying commands, never on drag-frequency ones.
