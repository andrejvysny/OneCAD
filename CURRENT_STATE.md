...
**VF-M5 CLOSED (WP0).** The gate's discriminator (`job.partition.size() == 0`) cannot express the hazard it guards: the RegenPlanner emits full-replay-from-0 plans for EVERY regen and `Session::fence_and_clone` clones an empty base for those (D5), so it is always true and the gate degenerated to `editedFrom.is_some()` — i.e. to any user edit, which is why the flagship fillet lane came back `NeedsRepair`. The real hazard needs a RESTORED basis, which V1 cannot produce. Resolution (a): `from_zero_replay = false`, with the derivation and the re-enable condition recorded at the assignment; `ScratchJob.h`/`Ladder.h` re-document the field as what it always meant. Reproduced RED on a HEAD-built worker first, green after.
