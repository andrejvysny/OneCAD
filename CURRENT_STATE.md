...
- **Changed files (by package):** WP00 `docs/viewport-hardening/execution/STATUS.md`, `docs/qa/viewport-hardening/**` (baseline logs, runs, matrix, fixtures), `scripts/verify-viewport-acceptance.mjs`, `docs/adr/0015-*`, `src/viewport/vph/{instrumentation.ts,baselineCounterexamples.test.ts}`, `e2e/vph-{line-width,hover-lifecycle}.spec.ts` · WP01 `src/viewport/engine/{dpr,ViewportMetrics,screenLineStyle,dprWatcher,bodyMaterials,HighlightLayer,SketchObject,SketchStaticLayer,SnapIndicator,OriginTriad,Picker,ViewportEngine}.ts` + README · WP02 `engine/{FrameScheduler,ViewportEngine,renderer}.ts`, `src/viewport/ViewportRoot.tsx`, `src/features/settings/SettingsModal.tsx` (+test) · WP03 `src/viewport/mesh/{meshRegistry,renderResourceIdentity,faceSliceGeometry,highlightCache}.ts`, `engine/{HighlightLayer,BodyObject,SectionLayer}.ts` · WP04 `src/viewport/mesh/{validateMesh,meshAdmission,meshMutations,previewMesh,meshSync}.ts`, `src/ipc/promote.ts`, preview sites in `ModelToolController.ts` / `placementController.ts` · WP08 `worker/src/tess/{CurveSampler.h,CurveSampler.cpp,Tessellate.cpp}`, `worker/tests/test_curve_sampler.cpp` · WP09 `worker/src/tess/{SurfaceNormals,EdgeClassification}.{h,cpp}`, `Tessellate.{h,cpp}`, `worker/tests/test_surface_normals.cpp` · `docs/design/astra/wp08-curve-sampler-{break,followup}.md` · `TODO.md`, `HANDOFF.md`, this file.
- **Build/test (orchestrator-run at handoff):** `bunx tsc --noEmit` → pass · `bunx vitest run src/viewport src/ipc/promote.test.ts` → 59 files / 969 passed / 6 expected fail · worker focused `ctest -R "surface_normals|curve_sampler|tessellation_quality|wp5_mesh1|wp6_meshexport|parity|canonical"` → 45/45 · full `ctest` → 196/198 (WP08 tree, orchestrator) and 197/199 (WP09 tree, implementer), both only the baseline reds `feature_pattern`, `chamfer_reference_face` · Rust edge consumers (7 targets, `--no-fail-fast`) → 45 passed / 3 failed, all baseline · Playwright chromium probes → line-width 3/3, hover plateau at 13 geometries · hex gate 0 · `node scripts/verify-viewport-acceptance.mjs` → 127 ok (26 focused-pass). **Not run after the changes:** full `bun run test` (baseline 443 pre-existing failures outside the viewport), full `cargo test --workspace` (baseline 24 failures), `bun run e2e` both projects, any native lane. Worker sidecar staged (`d3b5e01312788db4…`), stdout hygiene clean.
- **Key decisions:** harden in place, no renderer rewrite (ADR-0015) · baseline reds preserved and attributed, never repaired by this program · CSS px for all fat lines because the installed addon overwrites `resolution` with logical px (WP01) · submission record derives from displayed entries, not the adopted publication (WP02) · only a branded `ValidatedMesh` becomes GPU geometry; v1 header-bbox enclosure measured not fatal (WP04, DEV-WP04-1 routed to WP10) · registry is the unique disposer; leases + owned slices (WP03, D05) · curve sampler: chord certificate is the acceptance, angular evidence is honest (`Undefined` when incomplete); Astra followup says the emitted-segment turn predicate must become the acceptance rule and the 1e-9 floor must be replaced (WP08 round 3) · normals: `orientationSign · sign(det A)` once, winding `reversed XOR det<0` (WP09) · Astra runs authorized by the kickoff instruction for reviews; every packet, hash and transcript audit recorded.
- **Blockers:** none environmental. Owed before any commit: WP08 round 3, WP09 review + Astra break, WP03 round 2, then the full L3 rung compared against the baseline red sets. Port 4177 is held by another session's Vite — use `E2E_PORT=4179`. Rate limit killed two agents once (relaunched; WP03 first attempt had made no edits).

---

# CURRENT UX HARDENING — HISTORICAL HANDOFF (2026-09-12; superseded above)
