# CURRENT STATE — session 37 (2026-09-17/18, modeling interaction fix + harden) — HANDOFF

Last verified: 2026-09-18 09:10 (this session). Entry point: `HANDOFF.md`. Plan: `~/.claude/plans/analyze-this-report-and-bright-axolotl.md`. Ledger: `TODO.md` § SESSION 37. Astra design: `docs/design/astra/modeling-handle-attachment.md`.

- **Branch:** `master` at **`a988ecae`** (Boundary 1, committed on top of `9f573ecd`), **6 commits ahead of `origin/master`**. Push not authorized. Boundary 2 is UNCOMMITTED in the working tree: ~60 files, +5069/−983 in `src/tools/preview`, `src/viewport`, `src/tools/modelTools`, `src/features`, plus 18 migrated e2e specs. `.claude/agent-memory/**` is modified as always and is never staged.
- **Source:** the external review of `9f573ecd` (`~/Downloads/OneCAD-Modeling-Implementation-Review-9f573ecd.md`) against the acceptance spec (`~/Downloads/OneCAD-Modeling-Interaction-Unification-Spec.md`). Every review finding was re-verified against source: R01–R08 and §4.1–4.4 all confirmed, nine more found (N1–N9). Four adversarial reviews and one Astra `break`-class pass then found 9 + 8 + 13 further defects inside the fix packages; all fixed.
- **Boundary 1 gate of record (COMMITTED, `a988ecae`)** — main thread, nothing else running: tsc ✓ · build ✓ · `bun run test` **368 files / 6469 passed / 0 failed / 78 skipped** · hex **0** · coverage **34 / 9 / 17 / 20** · contracts **41 / 19 / 15** · self-test ✓ · e2e `retries: 0` **chromium 282 / 0** (21.2 min) and **webkit 282 / 0** (29.0 min) — `9f573ecd` itself was 277/5 and 279/3 · native tauri-agent pass, foreground `real_user`/`cgevent`, 14 checks, 0 error log lines.
- **Boundary 2 gate so far (UNCOMMITTED tree)** — main thread: tsc ✓ · build ✓ · `bun run test` **368 files / 6595 passed / 0 failed / 78 skipped** · hex **0** · coverage **34 / 9 / 17 / 20** · contracts **41 / 19 / 15** · self-test ✓ · **e2e chromium 282 / 0** (33.2 min, `retries: 0`). **e2e webkit was RUNNING when this snapshot was written** — log `/private/tmp/claude-501/-Users-andrejvysny-workspace-OneCAD/5cfb5ae3-bade-4763-8e88-38905cb5c03d/scratchpad/gate-b2-webkit.log`; if that scratchpad is gone, just re-run the lane.
- **Key decisions:** D-S1 hardening before spec completion · D-S2 symmetric span clamps at 0 keeping its stored sign · D-S3 Fillet and Chamfer both grow along the arrow · D-S4 Astra approved (derive accepted; `break` packet composed, NOT yet run) · the strip's mode controls stay readouts for now, live dropdowns owed as H7b · the tangent rejection is deleted outright (β = 0) on Astra's proof, which supersedes the earlier conditioning-gate fix · interval arithmetic adopted in shape only (relative guards, no γ-certificate machinery) · the glyph draws the true motion direction, so no tangent-perpendicular decoration.
- **Blockers:** none technical. Waiting on the user for: the Astra `break` run (approval summary printed, packet `381ad5e5411e`), and the Boundary 2 native pass, which takes the real mouse and keyboard.
- **Not run / owed:** webkit lane result (in flight); Astra `break`; Boundary 2 native pass + commit; H7b live mode dropdowns; Boundary 3 entirely (Revolve angular handle, Linear/Circular pattern handles, Move gizmo on shared primitives + its README supersession, Hole/Datum/Gear per §8.9, lifecycle A17/A18); the user-run manual checklist; the merged-stack Tauri smoke; "STEP opens coloured elsewhere"; "3MF opens in a slicer".
- **Harness defects found (tools/tauri-agent, not the app):** `session_start {mode:"launch"}` fails deterministically — the identity probe races the dev page load and reads `window.__TAURI__.core` before it exists (a direct WebDriver probe against the same app shows it present); and `{mode:"attach"}` refuses a port owner whose executable path is relative (`matchesApp:false` for `target/debug/onecad`). Workaround that works: start Vite, run `src-tauri/target/debug/onecad` by ABSOLUTE path with `TAURI_WEBDRIVER_PORT=4445`, then attach.
- **Processes:** the webkit Playwright lane was the only heavy job running; no dev server, no app, no worker left by this session (the native pass was torn down and ports 1420/4445/4177 verified free afterwards).
